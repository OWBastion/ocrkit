...
if [[ "$device" == cuda ]]; then
  "$python_bin" -m pip uninstall -y paddlepaddle paddlepaddle-gpu >/dev/null 2>&1 || true
  "$python_bin" -m pip install --no-deps "$paddle_package" -i "$paddle_index"
  "$python_bin" -c 'import paddle; assert paddle.is_compiled_with_cuda(), "installed PaddlePaddle is not CUDA-enabled"; assert paddle.device.cuda.device_count() > 0, "no CUDA device is visible to PaddlePaddle"; paddle.device.set_device("gpu:0"); value = paddle.to_tensor([1.0]).numpy()[0]; assert value == 1.0; print(f"PaddlePaddle {paddle.__version__}: CUDA {paddle.device.get_device()}")'
fi
